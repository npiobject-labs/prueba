...
| D5 | Título y etiquetas: los genera el backend al guardar, llamando al LLM del usuario con un prompt cerrado que devuelve JSON `{titulo, etiquetas[]}`. Vocabulario de etiquetas controlado: se le pasan las 40 etiquetas más usadas para que reutilice antes de inventar. | **Hecho (F2)**. El proveedor es la pasarela del usuario sobre OpenRouter (endpoint estilo OpenAI `/v1/chat/completions`; la URL concreta no se escribe aquí porque `docs/` es público: va en la variable de repositorio `LLM_URL`). Todo el contrato vive en `app/src/llm.rs` y se configura por entorno (`LLM_URL`, `LLM_MODELO`, `LLM_API_KEY`, `LLM_CABECERA`, `LLM_PREFIJO`, `LLM_ESPERA`), así que cambiar de API es cambiar variables. Confirmado con la documentación del proveedor: cuerpo `{messages, max_tokens}`, clave en `Authorization: Bearer` y cabecera de traza `X-Operacion`. `model` solo se envía si se fija `LLM_MODELO`, porque la pasarela ya elige modelo. **Pendiente**: el usuario crea las variables y el secreto. |
...
- **Encender la IA (D5/D9)**: en GitHub → Settings → Secrets and variables → Actions, crear la *variable* `LLM_URL` (endpoint `/v1/chat/completions` de la pasarela), el *secreto* `LLM_API_KEY` (la clave de la app «prueba»), y opcionalmente `LLM_MODELO` (si se quiere forzar modelo) y `LLM_OPERACION` (etiqueta de traza). El siguiente run de `deploy.yml` los pasa a Fly; `/salud` mostrará `"ia":true`. Sin `LLM_URL` todo sigue funcionando con el título de respaldo.
