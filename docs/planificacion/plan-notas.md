...
Fecha: 2026-09-20 · Estado: v13, F0 a F12 hechas: notas dictadas, tituladas por IA, buscables, con token, y documentos generados a partir de una selección de notas · Fuente de verdad: este fichero (la copia en Drive es solo copia) · Lo siguiente, notas por proyectos (F13 a F17), está en `plan-proyectos.md`.
...
| D4 | Dictado: en el navegador con Web Speech API (`SpeechRecognition`, `lang=es-ES`), resultado en el mismo `textarea`. | El supuesto de que Chrome/Android era el uso principal se cumplió, y allí **falló**: ver F8. Sin `continuous`, recorriendo desde `resultIndex` y con el dictado largo por rearranque. Plan B de iOS sin tocar: allí no hay `SpeechRecognition` y se usa el micro del teclado del sistema. |
...
| **F8** ✅ | Arreglo del dictado en Android (mock 9, `PR-B1-20260920-011`): el texto salía repetido palabra a palabra porque Chrome en Android entrega cada versión del resultado interino en un índice nuevo y se sumaban todos. Ahora se recorre desde `ev.resultIndex`, el interino no se acumula, los trozos finales se fusionan solo si llegan pegados (2 s) y el dictado largo sale de rearrancar en `onend` en vez de `continuous`. | Ocho casos con el motor de voz simulado en Chromium: las dos formas del fallo de Android, el comportamiento normal de escritorio, el rearranque tras un corte del navegador, que Parar no rearranca, que respeta el texto ya escrito, que dos frases iguales pegadas se fusionan pero separadas cuentan dos veces, y el mensaje cuando se deniega el micrófono. Falta la prueba en el móvil de verdad, que es donde salió el fallo. |
| **F9** ✅ | Mock 10 (`PR-B1-20260920-012`): botón **Todas / Ninguna** en la barra del modo selección. Opera sobre lo que hay en pantalla, no sobre la base entera: con filtro de etiqueta o búsqueda activos marca justo lo visible, y la etiqueta del botón cambia a «Ninguna» cuando ya están todas. El tope de 20 notas no se toca: si lo visible lo pasa, el contador lo avisa en rojo y Generar sigue bloqueado. La barra pasa a dos filas (contador arriba, tres botones abajo) para que quepan en móvil. | Once comprobaciones con Chromium a 390×844: marcar y vaciar, desmarcar una y volver a completar, la etiqueta del botón en cada estado, «Todas» con filtro de etiqueta y con búsqueda, la selección previa intacta al quitar el filtro, el botón deshabilitado con la lista vacía, 25 notas → aviso de tope y Generar bloqueado, la barra sin desbordar a 390 y a 900 px, y la última nota alcanzable bajo la barra más alta. Sin errores de consola. |
| **F10** ✅ | Mock 10 (`PR-B1-20260920-012`): la tira de etiquetas del buscador se pliega. Un botón «Etiquetas · N ▾» la abre y la cierra, cerrada por defecto, y la elección se recuerda en `localStorage` (`notas-etiquetas`). Con filtro puesto el botón enseña la etiqueta activa en vez del recuento, para que una lista recortada nunca quede sin explicación a la vista. Sin etiquetas, el botón no aparece. | Chromium a 390×844: con 15 etiquetas la cabecera baja de 295 a 139 px; plegar y desplegar, la flecha y el `aria-expanded` en cada estado, la etiqueta activa en el botón al filtrar y al plegar, el estado recordado al recargar en los dos sentidos, el botón oculto sin etiquetas, y el tema oscuro. Las once comprobaciones de F9 se repitieron sobre este cambio. Sin errores de consola. |
| **F11** ✅ | Exportación (mock 11, `PR-B1-20260920-013`): botón en Ajustes que descarga todas las notas en un `.md` con título, fecha, etiquetas y contenido, con Compartir donde el navegador lo ofrece. La descarga de un documento y esta comparten la misma función. | Probado con Chromium: el fichero baja como `notas-AAAA-MM-DD.md`, trae una sección por nota, y si el backend no responde el aviso lo dice y el botón vuelve a su sitio. |
| **F12** ✅ | Mock 12 (`PR-B1-20260920-014`): la app avisa de que hay versión nueva con una banda y un botón «Actualizar», buscándola al abrir y al volver a ella; en Ajustes, el build a la vista y «Buscar actualización». Y `pages.yml` deja de dar por bueno el despliegue con el artefacto subido: hace `curl` al sitio publicado y falla si el build servido no es el del commit, o si lo que sirve es el README (Source mal configurado). | Chromium: publicada una versión nueva en un sitio de prueba, la banda aparece al volver a la app, «Actualizar» recarga con el build nuevo y la banda desaparece. La verificación de Pages se prueba sola en cada publicación: si no pasara, el run se pondría rojo. |
...
El 2026-09-23 el usuario pidió que las notas se guarden además por proyectos (abrir un proyecto y dictar dentro; buscar y ordenar proyectos). Está planificado, no implementado, en [`plan-proyectos.md`](plan-proyectos.md): decisiones D14 a D26 y fases F13 a F17, con numeración continua a la de aquí.
