...
- Cajas redimensionables (F28): todo `textarea` con id lleva un asa propia (`.asa`, en su esquina inferior derecha) que se arrastra con el dedo o el ratón, porque el `resize` nativo no sale en Chrome de Android ni en Safari de iOS (`.conAsa` lo apaga para no tener dos). La altura se recuerda por id en `localStorage` (`notas-alturas`) y dos toques en el asa vuelven a la de serie. Un `textarea` nuevo necesita `id` para tenerla.
