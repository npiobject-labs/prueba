...
- Proyectos (F13 a F16, D14 a D26 en `docs/planificacion/plan-proyectos.md`): `GET/POST /proyectos`, `GET/PATCH/DELETE /proyectos/{id}`, `PATCH /notas/{id}` y `POST /notas/mover`; filtro `proyecto=<id>|ninguno` en `/notas`, `/etiquetas` y `/documentos`, y `proyecto` al crear nota. Una nota está en un proyecto como máximo (`notas.proyecto_id`, nullable); `null` es «Sin proyecto». Borrar un proyecto **nunca** borra notas ni documentos. El esquema se versiona con `PRAGMA user_version` (función `migrar` en `main.rs`): un cambio que no quepa en `CREATE ... IF NOT EXISTS` va en una versión nueva. `GET /notas` sin filtro sigue devolviendo todas, para que la app cacheada siga funcionando. En la app, el proyecto activo se recuerda en `localStorage` (`notas-proyecto`) y el orden de la lista de proyectos en `notas-orden-proyectos`. Las secciones **📁 Proyectos · 📝 Notas · 🎙 Entrevistas** están en el menú ☰ (ver «Cabecera»): Notas vuelve al proyecto activo o, sin ninguno, enseña todas las notas; «Editar proyecto» también está en el menú. Crear es igual en las tres secciones: el botón flotante ＋ de abajo a la derecha (`#nueva`) hace nueva nota, nuevo proyecto o nueva entrevista según la pestaña, y cambia `title`/`aria-label` en `ver()`; en el editor de una nota, el desplegable **Proyecto** parte del proyecto activo, y si se guarda en otro la app entra en ese proyecto.
- Subcarpetas (F23/F24, D43 a D53 en `docs/planificacion/plan-subproyectos.md`): un proyecto puede tener subcarpetas (`proyectos.padre_id`, migración 3), **un solo nivel** (400 si el padre ya es subcarpeta). Una nota, un documento o una entrevista van al proyecto o a una de sus subcarpetas y, si es subcarpeta, traen `proyecto.padre`. `proyecto=<id>` en `/notas`, `/etiquetas`, `/documentos` y `/entrevistas` **incluye las subcarpetas**; `solo=1` las quita («Sin subcarpeta»). `POST/PATCH /proyectos` aceptan `padre` (id o `null`; en `PATCH`, ausente = no se toca); `GET /proyectos` devuelve las subcarpetas como filas con `padre`, y en un principal `notas` suma las de sus subcarpetas (`notas_propias`, `subcarpetas`). En las subcarpetas `nombre_clave` lleva delante el id del padre: el nombre es único dentro de cada padre sin reconstruir la tabla. Archivar es de principales y arrastra a sus subcarpetas. Borrar una subcarpeta sube lo suyo al padre; borrar un principal manda lo suyo a «Sin proyecto» y deja sus subcarpetas como principales (renombradas «X (Padre)» si chocan). En la app: tira **Todo · Sin subcarpeta · 📂 …** sobre la lista, solo si el proyecto tiene subcarpetas (`#carpetas`; crear una es «Nueva subcarpeta» en el menú), migas «Padre ›» en el título de la barra (`#migas`), chips 📂 en las tarjetas de Proyectos, «Dentro de» y lista de subcarpetas en la hoja del proyecto, y árbol en «Mover a…» y en el editor (rutas «Padre › Subcarpeta»).
- Entrevistas (F18 a F21, D27 a D42 en `docs/planificacion/plan-entrevista.md`): `GET/POST /entrevistas` (`POST` multipart: `audio`, `proyecto`, `con_quien`, `duracion_s`, `titulo`; hasta 200 MB, volcado a disco según llega), `GET/PUT/DELETE /entrevistas/{id}`, `GET/DELETE /entrevistas/{id}/audio` (con `Range`; borrar solo el audio exige la transcripción hecha), `POST /entrevistas/{id}/transcribir`, `/resumir`, `/nota` (`{cual: ejecutivo|amplio|ambos}`) y `/mover`; filtro `proyecto=` y `textos=1` (lo usa la exportación). El código vive en `app/src/entrevistas.rs`; el audio, en `audio/` junto a la base (en Fly, `/data/audio`). Transcribir: `ffmpeg` (va en la imagen de Docker) trocea en mp3 de 5 min y cada trozo va al modelo como `input_audio` por el mismo proxy (1,2 MB por petición, bajo los 2 MB del proxy); los trozos se guardan en `entrevista_trozos`, así que «Reintentar» sigue desde el que falló. Resumir: una llamada con salida estructurada que da el ejecutivo y el amplio; el markdown del amplio lo compone el backend y el título del modelo no pisa uno editado a mano. Todo en segundo plano con sondeo cada 3 s, como los documentos; al arrancar, lo que estaba a medias pasa a fallido. En la app: sección **🎙 Entrevistas** (en el menú) y pantalla de captura a pantalla completa (Grabar/Parar, Pausa, Cerrar) con `MediaRecorder`, vibración, Wake Lock, Media Session con un audio silencioso en bucle de 10 s (Chrome quita los controles a lo que dura 5 s o menos) que suena desde «Listo para grabar» hasta parar o cerrar, también en pausa, y notificación con Pausa/Parar que `sw.js` reenvía a la página. Un solo botón de medios (auriculares, mando Bluetooth o el doble toque trasero del Pixel, Quick Tap → «Reproducir o pausar contenido multimedia») graba, pausa y sigue: `play` y `pause` van al evento `medios` y decide el estado (D77); parar es solo en pantalla o en la notificación. Volumen y encendido no llegan nunca a la página. Lo grabado se guarda en IndexedDB (`notas-grabaciones`) a trozos de 10 s: si la pestaña muere, el menú lo avisa (⚠️ en Entrevistas y un punto rojo en ☰) y la lista ofrece Subir o Descartar. El aviso de consentimiento sale una vez (`notas-consentimiento`). Acceso directo del manifest → `index.html?grabar=1`: con el micro ya permitido arranca solo tras 3 s, y un toque lo cancela. En el detalle, Audio, Transcripción y los dos resúmenes son `details.plegable` con la misma flecha ▾ a la derecha (helper `plegable()`); lo que el usuario pliega a mano se recuerda en `plegEnt` mientras siga en esa entrevista, para que el sondeo no se lo deshaga. **Nunca reutilices ids de la parte de notas**: el detalle usa `ent*` porque `eTitulo` y `eProyecto` son del editor.
- Informes con roles (F25/F26, D54 a D67 en `docs/planificacion/plan-analisis.md`): un documento es `tipo: encargo` (lo de siempre) o `informe`. `POST /documentos` acepta `tipo`, `roles` (ids, de 1 a 8) y `modelo` (del catálogo; vacío = el del servidor; en un encargo tiene que dar salida estructurada, si no 400). Cada rol va en `documento_roles` con su copia (nombre, icono, enfoque) y estado `espera|trabajando|hecho|fallido`; se hacen 4 a la vez y luego una síntesis en markdown (sin `json_schema`, para que valga cualquier modelo) que escribe título, resumen ejecutivo, conclusiones, tensiones entre roles, plan de acción y preguntas. El backend compone el informe (síntesis + «Análisis por rol» + «Notas de origen» + «Ficha») y lo guarda en `texto`. Si un rol falla, el informe sale con los demás y `POST /documentos/{id}/reintentar` rehace solo los fallidos y la síntesis; al arrancar, lo que estaba a medias pasa a fallido. `regenerar` acepta `tipo`, `roles` y `modelo` y sube `generacion`; el coste de la ficha sale de `/v1/uso/resumen?operacion=informe-<id>-<generacion>` del servicio. Roles: nueve de serie en `app/src/analisis.rs` (`DE_SERIE`) y propios en la tabla `roles` (`GET/POST /roles`, `PUT/DELETE /roles/{id}`; tocar uno de serie da 400). `GET /modelos` reenvía el catálogo del servicio (solo modelos de texto, por nombre, con `recomendado` y `defecto`). Migración 4. `deploy.yml` lo verifica en «Verificar informes». En la app: la hoja de Generar elige 📄 Encargo o 🔎 Análisis (recordado en `notas-generar`), roles en tarjetas (última elección en `notas-roles`, tope 8, «＋ Rol propio» lo crea y lo marca), modelo (el de ⚙ Ajustes, en `notas-modelo`, o uno solo para esa hoja) y coste estimado (amarillo por encima de 0,25 $; un encargo con un modelo sin salida estructurada queda bloqueado). El documento enseña ficha (tipo · modelo · coste), progreso rol a rol, aviso con «Reintentar» si falló algún rol y el texto en **Leer** (markdown pintado por `markdown()`, propio y sin librerías, porque la PWA funciona sin red) o **Editar**, con 📋 Copiar arriba y abajo, ⬇ .md y 🖨 PDF (`@media print` deja solo `#gMd`). En Ajustes, el modelo por defecto y los roles: los de serie se ven y se duplican; los propios se editan y se borran. `api()` añade al error el mensaje del backend (`HTTP 400: …`), así que las comprobaciones con `/HTTP 404/` siguen valiendo.
...
- Cajas redimensionables (F28): todo `textarea` con id lleva debajo un asa propia (`.asa`: barra a todo lo ancho con una rayita en el centro) que se arrastra con el dedo o el ratón, porque el `resize` nativo no sale en Chrome de Android ni en Safari de iOS (`.conAsa` lo apaga para no tener dos). La altura se recuerda por id en `localStorage` (`notas-alturas`) y dos toques en la barra vuelven a la de serie. Un `textarea` nuevo necesita `id` para tenerla.
...
- Cabecera (F27, D68 a D76 en `docs/planificacion/plan-menu.md`): una sola barra fija de 56 px con ☰ (`#menuBtn`), título en dos pisos (`#seccion` arriba y `#cab` abajo, con `#migas`; lo pinta `pintarTitulo()` según `vistaActual`), estado, 🔍 (`#buscarBtn`) y tema. El menú (`#menu`) tiene *Ir a* (`#tabProy`, `#tabNotas`, `#tabEnt`, con cifras), *Este proyecto* (`#seleccionarBtn`, `#documentosBtn`, `#cpEditar`, `#nuevaSubBtn`; sin proyecto se llama «Notas») y *General* (`#ajustesBtn`). Se cierra al elegir (cada botón hace lo suyo y un listener del menú lo cierra), al tocar fuera, con Escape y con **Atrás**: abrirlo hace `history.pushState` y `popstate` lo cierra, así que Atrás no saca de la app. La lupa es de Notas (`#buscaNotas`: texto, día, etiquetas y «en todos») y de Proyectos (`#buscaProy`: texto y orden); en las demás pantallas no sale. Con un filtro puesto y el buscador cerrado, `#resumenFiltro` lo resume («🔍 "x" · #etiqueta · 3 de 15») y ✕ lo quita, y la lupa queda en color. La primera fila de cada pantalla son sus cifras: `#cabProyecto` en la lista (solo se pinta con la lista ya cargada para ese ámbito, `ambitoCargado`, para no enseñar cifras de otro), `#cifrasProy`, `#cifrasEnt` y `#cifrasDocs`. En la selección, ☰ pasa a ✕ y el título dice «ELEGIR NOTAS / N de M»; «Seleccionar notas» desde otra pantalla va antes a la lista (`seleccionarAlCargar`).
