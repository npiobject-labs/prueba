...
- Título y etiquetas por IA (F2, D5/D6/D9): `app/src/llm.rs` llama al servicio propio `npiobject-labs/openrouter` (contrato de OpenAI) con `POST {LLM_URL}/v1/chat/completions`, `Authorization: Bearer <LLM_CLAVE>`, `X-Operacion: notas-titulo` y salida estructurada. `LLM_CLAVE` es una **clave de aplicación** de ese servicio (secreto de repositorio, lo vuelca `deploy.yml`); `LLM_URL` (variable, por defecto el VPS `https://apisor.oracle402.com`; Fly es `https://openrouter-npiobject-labs.fly.dev`) y `LLM_MODELO` (variable, opcional) mandan a dónde y con qué modelo. Sin `LLM_CLAVE` no hay IA y `/salud` responde `"ia":false`. La nota se guarda **siempre antes** de llamar, con título de respaldo y `pendiente_ia`; 8 s de espera y `POST /notas/{id}/reintentar-ia` para el reintento. Guía: `docs/planificacion/titulado-ia.md`.
